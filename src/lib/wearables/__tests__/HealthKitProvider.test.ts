...
describe('HealthKitProvider', () => {
  ...
  describe('isAvailable', () => {
    it('reflete o resultado real do plugin', async () => {
      vi.mocked(Health.isAvailable).mockResolvedValue({ available: true, platform: 'ios' });
      await expect(provider.isAvailable()).resolves.toBe(true);
    });

    it('retorna false quando HealthKit não está disponível (ex: iPad sem suporte)', async () => {
