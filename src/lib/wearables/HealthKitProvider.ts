/**
 * Único arquivo desta etapa que fala com HealthKit de verdade — via
 * @capgo/capacitor-health (mesmo plugin já usado no provider Android).
 * Implementa exatamente o contrato WearableProvider da Etapa 2; nada fora
 * daqui (service, store, telas) sabe que HealthKit existe.
 *
 * Espelha HealthConnectProvider.ts 1:1 (mesmo SCOPE_TO_DATATYPE, mesmo
 * padrão de guarda por escopo via checkAuthorization) — o plugin já
 * normaliza HealthKit/Health Connect pra API idêntica, então a única
 * diferença real entre os dois arquivos é `id`/`platform`.
 *
 * Escopo: leitura (passos, distância, calorias ativas, FC, FC em repouso,
 * sessões) + disponibilidade + permissões granulares. writeSession existe
 * só por exigência da interface — grava como amostra de calorias, mesma
 * aproximação já documentada no provider Android.
 */
...
export class HealthKitProvider implements WearableProvider {
  readonly id = 'apple-healthkit';
  readonly platform: PlatformId = 'ios';
