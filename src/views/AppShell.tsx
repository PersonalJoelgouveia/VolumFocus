...
export function AppShell() {
  ...
  // Restringe views `ptOnly` ao Personal — reforço em nível de renderização,
  // além do já existente esconder os botões de nav (Sidebar/BottomNav só
  // listam PT_NAV quando isPersonalMode). Cobre o caso de `activeView` ficar
  // com uma view PT-only selecionada de uma sessão anterior (ex.: PT sai e
  // um aluno loga no mesmo dispositivo) — index.html não tinha esse caso
  // porque o DOM das views `pt-only` era removido/reescondido no logout.
  useEffect(() => {
    if (meta.ptOnly && !isPersonalMode) setActiveView('registro');
  }, [meta.ptOnly, isPersonalMode, setActiveView]);

  const safeView = meta.ptOnly && !isPersonalMode ? 'registro' : activeView;
